...
describe('<Header />', () => {
  ...
  test('active tab state changes on click', () => {
    ...
    Object.values(Tab).forEach((label) => {
      ...
      expect(link.closest('a')).toHaveStyle(`border-bottom: 5px solid ${colors.gray080}`);
    });

    // Check if the active tab state changes correctly
    const firstTab = screen.getByText(Object.values(Tab)[0]);
    fireEvent.click(firstTab);
    expect(firstTab.closest('a')).toHaveStyle(`border-bottom: 5px solid ${colors.gray080}`);

    const secondTab = screen.getByText(Object.values(Tab)[1]);
    fireEvent.click(secondTab);
    expect(secondTab.closest('a')).toHaveStyle(`border-bottom: 5px solid ${colors.gray080}`);
    expect(firstTab.closest('a')).not.toHaveStyle(`border-bottom: 5px solid ${colors.gray080}`);
  });
